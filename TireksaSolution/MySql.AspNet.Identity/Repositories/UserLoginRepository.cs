    ...
    public class UserLoginRepository
    {
        ...
        public void Insert(IdentityUser user, UserLoginInfo login)
        {
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                MySqlHelper.ExecuteNonQuery(conn, @"INSERT INTO userlogins(UserId,LoginProvider,ProviderKey) VALUES(@Id,@LoginProvider,@ProviderKey)", parameters);
            }
        }
        ...
        public void Delete(IdentityUser user, UserLoginInfo login)
        {
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                MySqlHelper.ExecuteNonQuery(conn, @"DELETE FROM userlogins WHERE UserId = @Id AND LoginProvider = @LoginProvider AND ProviderKey = @ProviderKey", parameters);
            }
        }
        ...
        public string GetByUserLoginInfo(UserLoginInfo login)
        {
            ...
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                var userIdObject = MySqlHelper.ExecuteScalar(conn, CommandType.Text,
                    @"SELECT UserId FROM userlogins WHERE LoginProvider = @LoginProvider AND ProviderKey = @ProviderKey", parameters);
        ...
        public List<UserLoginInfo> PopulateLogins(string userId)
        {
            ...
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                var reader = MySqlHelper.ExecuteReader(conn, CommandType.Text,
                   @"SELECT LoginProvider,ProviderKey FROM userlogins Where UserId = @Id", parameters);
                while (reader.Read())
                {
