...
namespace MySql.AspNet.Identity.Repositories
{
    public class RoleRepository<TRole> where TRole: IdentityRole
    {
        ...
        public IQueryable<TRole> GetRoles()
        {
            ...
            {
                var reader = MySqlHelper.ExecuteReader(conn, CommandType.Text, @"SELECT Id,Name FROM roles", null);
        ...
        private string GetRoleName(string roleId)
        {
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                var result = MySqlHelper.ExecuteScalar(conn, CommandType.Text, @"SELECT Name FROM roles WHERE Id = @id", parameters);
               if (result != null)
               {
        ...
        private string GetRoleId(string roleName)
        {
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                var result = MySqlHelper.ExecuteScalar(conn, CommandType.Text, @"SELECT Id FROM roles WHERE Name = @name", parameters);
                if (result != null)
                {
                    return result.ToString();
                }
