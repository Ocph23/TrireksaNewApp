    ...
    public class UserRepository<TUser> where TUser : IdentityUser
    {
        ...
        public void Insert(TUser user)
        {
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                MySqlHelper.ExecuteNonQuery(conn, @"INSERT INTO users VALUES(@Id,@Email,@EmailConfirmed,@PasswordHash,@SecurityStamp,@PhoneNumber,@PhoneNumberConfirmed,
        ...
        public IQueryable<TUser> GetAll()
        {
            ...
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                LockoutEndDateUtc,LockoutEnabled,AccessFailedCount,UserName FROM users", null);
        ...
        public TUser GetById(string userId)
        {
            ...
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                var reader = MySqlHelper.ExecuteReader(conn, CommandType.Text,
                    @"SELECT Id,Email,EmailConfirmed,
                PasswordHash,SecurityStamp,PhoneNumber,PhoneNumberConfirmed,TwoFactorEnabled,
                LockoutEndDateUtc,LockoutEnabled,AccessFailedCount,UserName FROM users WHERE Id=@Id", parameters);
                while (reader.Read())
                {
                    user.Id = reader[0].ToString();
                    user.Email = reader[1].ToString();
        ...
        public TUser GetByName(string userName)
        {
            ...
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                var reader = MySqlHelper.ExecuteReader(conn, CommandType.Text,
                    @"SELECT Id,Email,EmailConfirmed,
                PasswordHash,SecurityStamp,PhoneNumber,PhoneNumberConfirmed,TwoFactorEnabled,
                LockoutEndDateUtc,LockoutEnabled,AccessFailedCount,UserName FROM users WHERE UserName=@UserName", parameters);
                while (reader.Read())
                {
                    user.Id = reader[0].ToString();
                    user.Email = reader[1].ToString();
        ...
        public TUser GetByEmail(string email)
        {
            ...
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                var reader = MySqlHelper.ExecuteReader(conn, CommandType.Text,
                    @"SELECT Id,Email,EmailConfirmed,
                PasswordHash,SecurityStamp,PhoneNumber,PhoneNumberConfirmed,TwoFactorEnabled,
                LockoutEndDateUtc,LockoutEnabled,AccessFailedCount,UserName FROM users WHERE Email=@Email", parameters);
                while (reader.Read())
                {
                    user.Id = reader[0].ToString();
                    user.Email = reader[1].ToString();
